...
function EnhanceMarksheet(){
 useEffect(()=>{let cancelled=false;let timer:ReturnType<typeof setTimeout>|null=null
  const run=async()=>{const {data}=await supabase.from('site_settings').select('key,value').in('key',['logo_url','school_logo','school_name','school_address']);if(cancelled)return;const settings=Object.fromEntries((data||[]).map(x=>[x.key,x.value]));const logo=settings.logo_url||settings.school_logo||''
   const apply=()=>{if(cancelled||!window.location.pathname.startsWith('/admin/students/'))return;const sheet=document.querySelector<HTMLElement>('.marksheetPreview.bsebStyle');if(!sheet)return false;const header=sheet.querySelector('.boardHeader');const summary=sheet.querySelector('.sheetMeta');if(header){const seal=header.querySelector('.boardSeal');if(logo&&seal&&!header.querySelector('.schoolLogo')){const img=document.createElement('img');img.src=logo;img.className='schoolLogo';img.alt='School Logo';seal.replaceWith(img)}const small=header.querySelector('small');if(small&&settings.school_name)small.textContent=settings.school_name;const p=header.querySelector('p');if(p&&settings.school_address)p.textContent=settings.school_address}
   ...
   // The Results link uses a query parameter. Switch the profile to that tab once, without a MutationObserver.
