  ...
  const run=async()=>{
   ...
   try{
    ...
    const makeSheet=(g:any,index:number)=>{
     const cleanRows=g.rows||[];const full=cleanRows.reduce((n:any,x:any)=>n+Number(x.max_marks||0),0);const obtained=cleanRows.reduce((n:any,x:any)=>n+Number(x.marks||0),0);const pct=full?obtained/full*100:0;const overall=grade(pct);const final=result(pct);const badgeClass=final==='PASS'?'pass':'fail'
