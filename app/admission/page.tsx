...
export default function Admission(){
 ...
 async function submit(e:any){
  ...
  try{
   ...
   const form=new FormData();form.set('student_name',f.student_name.trim());form.set('dob',f.dob);form.set('class_applied',f.class_applied);form.set('father_name',f.father_name.trim());form.set('mother_name',String(f.mother_name||'').trim());form.set('phone',f.phone.trim());form.set('email',String(f.email||'').trim());form.set('address',f.address.trim());form.set('message',String(f.message||'').trim());form.set('student_photo',f.student_photo);form.set('identity_proof',f.identity_proof)
   let data:any=null,error:any=null
   for(let attempt=1;attempt<=2;attempt++){const r=await supabase.functions.invoke('submit-admission',{body:form});data=r.data;error=r.error;if(!error)break;if(attempt===1)await new Promise(r=>setTimeout(r,700))}
   if(error){let detail='';try{const response=(error as any).context;if(response?.json){const body=await response.json();detail=body?.error||''}}catch{}throw new Error(detail||error.message||'Unable to submit application. Please try again.')}
 ...
 return <div className="formPage"><div className="formHeader"><Link href="/">← New Sunrise Public School</Link><div><span>ADMISSIONS 2026–27</span><h1>Online Admission Application</h1><p>Nursery to Class 8 · Kalyangaon, Bihar</p></div></div>
